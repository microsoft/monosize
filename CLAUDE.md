...
- **Bundler adapter contract.** `BundlerAdapter.buildFixtures` is the batch path (3–15× faster); `buildFixture` is the sequential path used when the user passes `--build-mode=sequential`, which is mostly a debugging aid. Both return `{ name, outputDir, debugOutputPath? }` per fixture. `outputDir` is a per-fixture directory the CLI walks non-recursively; the JS bundle lives at `<outputDir>/index.js`. Debug output (when `debug: true`) is a beautified `*.debug.js` file written **alongside** `outputDir`, not inside it (so the extension scan doesn't double-count).
- **Asset types and report shape.** The CLI classifies files in `outputDir` by extension against `MonoSizeConfig.assetTypes` (closed `'js' | 'json' | 'css'` union; default `['js']`, opt in to `'css'`/`'json'` explicitly). Each report entry carries top-level `minifiedSize`/`gzippedSize` (sums) plus a `Partial<Record<AssetType, AssetSize>>` breakdown. Threshold gates on totals only; per-type deltas surface in the diff renderers (CLI sub-rows, markdown `### Breakdown` `<details>` blocks).
