...
describe('buildFixture', () => {
  ...
  describe('debug mode', () => {
    it('does not output additional files when disabled', async () => {
      ...
      const debugDir = buildResult.outputDir.replace(/\.output$/, '.debug');
    ...
    it('provides partially minified output when enabled', async () => {
      ...
      expect(fs.existsSync(path.join(buildResult.debugOutputDir as string, 'index.js'))).toBe(true);
    });
  });
});
...
describe('buildFixtures', () => {
  ...
  it('builds fixtures sequentially with debug', async () => {
    ...
    expect(fs.existsSync(path.join(buildResults[0].debugOutputDir as string, 'index.js'))).toBe(true);
    expect(fs.existsSync(path.join(buildResults[1].debugOutputDir as string, 'index.js'))).toBe(true);
  });
