...
describe('buildFixture', () => {
  ...
  describe('debug mode', () => {
    ...
    it('provides partially minified output when enabled', async () => {
      ...
      expect(buildResult.outputDir).toMatch(/monosize[\\|/]test\.output$/);
      expect(buildResult.debugOutputDir).toMatch(/monosize[\\|/]test\.debug$/);

      const output = await fs.promises.readFile(path.join(buildResult.outputDir, 'index.js'), 'utf-8');
      const debugOutput = await fs.promises.readFile(path.join(buildResult.debugOutputDir!, 'index.js'), 'utf-8');
...
describe('buildFixtures', () => {
  ...
  it('builds fixtures in batch mode with debug', async () => {
    ...
    expect(buildResults[0].debugOutputDir).toMatch(/monosize[\\|/]fixture1\.debug$/);
    expect(buildResults[1].debugOutputDir).toMatch(/monosize[\\|/]fixture2\.debug$/);

    // Verify debug files exist and contain the expected content
    const debugOutput1 = await fs.promises.readFile(path.join(buildResults[0].debugOutputDir!, 'index.js'), 'utf-8');
    const debugOutput2 = await fs.promises.readFile(path.join(buildResults[1].debugOutputDir!, 'index.js'), 'utf-8');
