# Change Log - monosize-bundler-webpack

<!-- This log was last generated on Thu, 07 May 2026 09:19:33 GMT and should not be manually modified. -->

<!-- Start content -->

## 0.4.0

Thu, 07 May 2026 09:19:33 GMT

### Minor changes

- BREAKING: adopt BundlerAdapter type changes (olfedias@microsoft.com)
- Bump monosize to v0.9.0

### Patches

- chore: bump terser from 5.46.2 to 5.47.0 (olfedias@microsoft.com)
- chore: bump terser from 5.46.0 to 5.46.2 (198982749+Copilot@users.noreply.github.com)

## 0.3.4

Mon, 27 Apr 2026 10:25:09 GMT

### Patches

- chore: bump webpack to v5.106.2 (olfedias@microsoft.com)
- chore: bump terser-webpack-plugin to v5.5.0 (olfedias@microsoft.com)
- Bump monosize to v0.8.3

## 0.3.3

Thu, 16 Apr 2026 09:05:06 GMT

### Patches

- Bump monosize to v0.8.2

## 0.3.2

Thu, 16 Apr 2026 08:04:24 GMT

### Patches

- fix: output full webpack compilation errors instead of truncated first line (olfedias@microsoft.com)
- Bump monosize to v0.8.1

## 0.3.0

Tue, 27 Jan 2026 13:37:07 GMT

### Minor changes

- feat: add support for batch build mode (pjsw@microsoft.com)
- Bump monosize to v0.8.0

## 0.2.1

Mon, 08 Dec 2025 12:45:18 GMT

### Patches

- Bump monosize to v0.7.1

## 0.2.0

Thu, 10 Jul 2025 14:01:30 GMT

### Minor changes

- chore: remove custom logging, add bundler name (olfedias@microsoft.com)
- Bump monosize to v0.7.0

## 0.1.6

Thu, 07 Nov 2024 14:43:21 GMT

### Patches

- chore: bump webpack (olfedias@microsoft.com)

## 0.1.5

Tue, 06 Aug 2024 20:40:08 GMT

### Patches

- Bump monosize to v0.6.3

## 0.1.4

Mon, 20 May 2024 09:09:44 GMT

### Patches

- Bump monosize to v0.6.2

## 0.1.3

Fri, 17 May 2024 08:39:46 GMT

### Patches

- Bump monosize to v0.6.1

## 0.1.2

Thu, 16 May 2024 15:04:43 GMT

### Patches

- Bump monosize to v0.6.0

## 0.1.1

Fri, 10 May 2024 09:04:46 GMT

### Patches

- Bump monosize to v0.5.1

## 0.1.0

Fri, 22 Mar 2024 14:27:41 GMT

### Minor changes

- feat:BREAKING CHANGE - change create*Bundler api to accept callback handler only (hochelmartin@gmail.com)
- Bump monosize to v0.5.0

## 0.0.2

Sat, 16 Mar 2024 11:26:36 GMT

### Patches

- chore: initial release (olfedias@microsoft.com)
- Bump monosize to v0.4.0
