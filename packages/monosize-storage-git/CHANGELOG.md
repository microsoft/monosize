# Change Log - monosize-storage-git

<!-- This log was last generated on Thu, 07 May 2026 12:28:07 GMT and should not be manually modified. -->

<!-- Start content -->

## 0.3.4

Thu, 07 May 2026 12:28:07 GMT

### Patches

- fix: skip artifacts whose JSON shape is not the expected `{ commitSHA, data }` wrapper, instead of returning `remoteReport: undefined` (olfedias@microsoft.com)

## 0.3.3

Thu, 07 May 2026 09:19:33 GMT

### Patches

- fix: bump adm-zip from 0.5.16 to 0.5.17 (198982749+Copilot@users.noreply.github.com)
- chore: bump @octokit/rest from 21.1.1 to 22.0.1 (49699333+dependabot[bot]@users.noreply.github.com)
- Bump monosize to v0.9.0

## 0.3.2

Mon, 27 Apr 2026 10:25:09 GMT

### Patches

- Bump monosize to v0.8.3

## 0.3.1

Thu, 16 Apr 2026 09:05:06 GMT

### Patches

- Bump monosize to v0.8.2

## 0.3.0

Wed, 11 Mar 2026 13:15:23 GMT

### Minor changes

- feat: add outputPath config to write report for actions/upload-artifact (olfedias@microsoft.com)

## 0.2.0

Fri, 06 Mar 2026 09:55:31 GMT

### Minor changes

- feat: rewrite to use GitHub Actions artifacts via Octokit (olfedias@microsoft.com)

## 0.1.0

Thu, 05 Mar 2026 18:15:03 GMT

### Minor changes

- feat: add monosize-storage-git package (olfedias@microsoft.com)
